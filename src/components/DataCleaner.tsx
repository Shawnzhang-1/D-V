    ...
    <div className={`card overflow-hidden ${className}`}>
      <div className="px-6 py-4 border-b border-gray-100">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-800">数据清洗</h2>
            <p className="text-xs text-gray-500">处理空值、格式转换、去重</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-600 mb-2">选择列</label>
          <select
        ...
        {columnStats && (
          <div className="mb-4 p-4 bg-gray-50 rounded-xl">
            <h4 className="text-sm font-medium text-gray-700 mb-2">列统计</h4>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              <div>
                <span className="text-gray-500">空值:</span>
                <span className="ml-1 font-medium text-red-600">{columnStats.nullCount}</span>
              </div>
              <div>
                <span className="text-gray-500">唯一值:</span>
                <span className="ml-1 font-medium">{columnStats.uniqueCount}</span>
              </div>
              {columnStats.min !== undefined && (
                <div>
                  <span className="text-gray-500">最小值:</span>
                  <span className="ml-1 font-medium">{columnStats.min?.toFixed(2)}</span>
                </div>
              )}
              {columnStats.max !== undefined && (
                <div>
                  <span className="text-gray-500">最大值:</span>
                  <span className="ml-1 font-medium">{columnStats.max?.toFixed(2)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'fillNull' && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-2">填充方式</label>
            ...
            {fillMethod === 'custom' && (
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-2">自定义值</label>
                <input
                  type="text"
                  value={customValue}
          ...
          <div className="space-y-4">
            <div className="p-4 bg-orange-50 rounded-xl border border-orange-200">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="text-sm font-medium text-orange-700">删除空值行</h4>
        ...
        {activeTab === 'convert' && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-2">转换类型</label>
              <select
        ...
        {activeTab === 'deduplicate' && (
          <div className="space-y-4">
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
              <h4 className="text-sm font-medium text-blue-700">全行去重</h4>
          ...
          <div className="space-y-4">
            ...
            {outlierInfo && outlierInfo.indices.length > 0 && (
              <div className="p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                <h4 className="text-sm font-medium text-yellow-700">检测到异常值</h4>
        ...
        {previewData && (
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-medium text-gray-700">预览结果 (前50行)</h4>
              <button onClick={() => setPreviewData(null)} className="text-sm text-gray-400 hover:text-gray-600">
                清除预览
              </button>
            </div>
