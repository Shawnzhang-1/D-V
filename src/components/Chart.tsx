...
const DEFAULT_COLORS = ['#5E81AC', '#81A1C1', '#88C0D0', '#8FBCBB', '#A3BE8C', '#EBCB8B', '#D08770', '#B48EAD'];
...
const CustomTooltip = memo<CustomTooltipProps>(({ active, payload, label, colorMap }) => {
  ...
  return (
    <div style={{ 
      backgroundColor: 'rgba(236, 239, 244, 0.98)', 
      backdropFilter: 'blur(12px)',
      padding: '12px 16px', 
      boxShadow: '0 10px 40px rgba(46, 52, 64, 0.12)', 
      borderRadius: '16px', 
      border: '1px solid #D8DEE9', 
      maxWidth: '280px' 
    }}>
      {displayLabel && (
        <p style={{ fontSize: '14px', fontWeight: 600, color: '#2E3440', marginBottom: '8px', borderBottom: '1px solid #D8DEE9', paddingBottom: '8px' }}>{displayLabel}</p>
      )}
      {limitedPayload.map((entry, index) => {
        ...
        const color = colorMap?.[displayName] || entry.color || '#5E81AC';
        return (
          <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', marginBottom: '4px' }}>
            ...
            <span style={{ color: '#4C566A' }}>{displayName}:</span>
            <span style={{ fontWeight: 600, color: '#2E3440' }}>
      ...
      {uniquePayload.length > MAX_TOOLTIP_ITEMS && (
        <p style={{ fontSize: '12px', color: '#4C566A', marginTop: '4px' }}>...还有 {uniquePayload.length - MAX_TOOLTIP_ITEMS} 项未显示</p>
      )}
    </div>
  );
});
      ...
      return (
        <div
          ...
          style={{ 
            ...
            borderRadius: '24px', 
            transition: 'all 0.3s',
            backgroundColor: isHidden ? '#E5E9F0' : '#ECEFF4',
            border: isHidden ? '1px solid #D8DEE9' : `1px solid ${config.color}40`
          }}
        ...
        >
          ...
          <span style={{ fontSize: '13px', color: isHidden ? '#4C566A' : '#2E3440', textDecoration: isHidden ? 'line-through' : 'none' }}>
            {key}
          </span>
        </div>
      );
...
const Chart: React.FC<ChartProps> = ({
  ...
  const getSeriesConfig = useCallback((key: string, index: number): { color: string; type: SeriesType; visible: boolean } => {
    ...
    return { 
      color: config?.color || colors[colorIndex], 
...
%%EOF`;
  ...
  const renderMainChart = useCallback((height: number = 400) => {
    const commonProps = { data: sampledData, margin: { top: 40, right: enableDualAxis ? 80 : 40, left: 60, bottom: 60 } };
    const xAxisComponent = (
      <XAxis dataKey={xKey} tick={{ fill: '#4C566A', fontSize: 11 }} axisLine={{ stroke: '#D8DEE9' }} tickLine={{ stroke: '#D8DEE9' }}>
        {xAxisLabel && <Label value={xAxisLabel} offset={-25} position="insideBottom" fill="#2E3440" fontSize={13} fontWeight={600} />}
      </XAxis>
    );
    const yAxisComponent = (
      <YAxis yAxisId="left" domain={['auto', 'auto']} tick={{ fill: '#4C566A', fontSize: 11 }} axisLine={{ stroke: '#D8DEE9' }} tickLine={{ stroke: '#D8DEE9' }} width={50}>
        {yAxisLabel && <Label value={yAxisLabel} angle={-90} offset={-25} position="insideLeft" fill="#2E3440" fontSize={13} fontWeight={600} style={{ textAnchor: 'middle' }} />}
      </YAxis>
    );
    
    const rightAxisLabel = dualAxisKeys.length > 0 
      ? dualAxisKeys.join(', ') 
      : (dataKeys.length > 1 ? dataKeys.slice(Math.ceil(dataKeys.length / 2)).join(', ') : '');
    
    const yAxisRightComponent = enableDualAxis && dataKeys.length > 1 ? (
      <YAxis 
        yAxisId="right" 
        orientation="right" 
        domain={['auto', 'auto']} 
        tick={{ fill: '#4C566A', fontSize: 11 }} 
        axisLine={{ stroke: '#D8DEE9' }} 
        tickLine={{ stroke: '#D8DEE9' }} 
        width={50}
      >
        {rightAxisLabel && <Label value={rightAxisLabel} angle={90} offset={-25} position="insideRight" fill="#2E3440" fontSize={13} fontWeight={600} style={{ textAnchor: 'middle' }} />}
      </YAxis>
    ) : null;
    
    const brushComponent = <Brush dataKey={xKey} height={30} stroke="#5E81AC" fill="rgba(94, 129, 172, 0.08)" />;
    ...
    switch (chartType) {
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <LineChart {...commonProps}>
              {showGrid && <CartesianGrid strokeDasharray="3 3" stroke="#E5E9F0" />}
              {xAxisComponent}
              {yAxisComponent}
              {yAxisRightComponent}
      ...
      case 'pie':
        return (
          <ResponsiveContainer width="100%" height={height}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={120} paddingAngle={2} dataKey="value" nameKey="name" label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`} labelLine={{ stroke: '#D8DEE9', strokeWidth: 1 }}>
                {pieData.map((entry, index) => <Cell key={index} fill={entry.color} stroke="#ECEFF4" strokeWidth={2} />)}
              </Pie>
              <Tooltip content={<CustomTooltip colorMap={colorMap} />} />
            </PieChart>
          </ResponsiveContainer>
        ...
        return (
          <ResponsiveContainer width="100%" height={height}>
            <RadarChart data={sampledData}>
              ...
              <PolarAngleAxis dataKey={xKey} tick={{ fill: '#4C566A', fontSize: 11 }} />
              <PolarRadiusAxis tick={{ fill: '#4C566A', fontSize: 11 }} />
              <Tooltip content={<CustomTooltip colorMap={colorMap} />} />
              {dataKeys.map((key, index) => {
                if (hiddenSeries.includes(index)) return null;
                const config = getSeriesConfig(key, index);
  ...
  if (data.length === 0) {
    return (
      <div style={{ backgroundColor: '#ECEFF4', borderRadius: '20px', border: '1px solid #D8DEE9', padding: '24px' }} className={className}>
        {title && <h3 style={{ fontSize: '18px', fontWeight: 600, color: '#2E3440', marginBottom: '16px' }}>{title}</h3>}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '320px', color: '#4C566A' }}><p>暂无数据</p></div>
      </div>
    );
  }
  ...
  return (
    <>
      <div style={{ backgroundColor: '#ECEFF4', backdropFilter: 'blur(16px)', borderRadius: '20px', border: '1px solid #D8DEE9', padding: '24px' }} className={className}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {title && <h3 style={{ fontSize: '18px', fontWeight: 600, color: '#2E3440' }}>{title}</h3>}
            <button onClick={() => setIsModalOpen(true)} style={{ padding: '8px', borderRadius: '12px', backgroundColor: '#E5E9F0', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} title="放大图表">
              <Maximize2 style={{ width: '16px', height: '16px', color: '#5E81AC' }} />
            </button>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {CHART_TYPES.map(({ type, label, icon }) => (
              <button key={type} onClick={() => setChartType(type)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 16px', borderRadius: '12px', fontSize: '13px', fontWeight: 500, border: 'none', cursor: 'pointer', transition: 'all 0.3s', background: chartType === type ? 'linear-gradient(135deg, #5E81AC, #81A1C1)' : '#E5E9F0', color: chartType === type ? '#fff' : '#4C566A' }}>
        ...
        <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #D8DEE9' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '8px', fontSize: '12px', color: '#4C566A' }}>
      ...
      {isModalOpen && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(46, 52, 64, 0.5)', backdropFilter: 'blur(8px)', padding: '16px' }}>
          <div ref={modalContentRef} data-chart-container style={{ backgroundColor: '#ECEFF4', backdropFilter: 'blur(20px)', borderRadius: '24px', border: '1px solid #D8DEE9', boxShadow: '0 25px 50px -12px rgba(46, 52, 64, 0.3)', width: '100%', maxWidth: '1280px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px', borderBottom: '1px solid #D8DEE9', background: 'linear-gradient(135deg, #E5E9F0, #ECEFF4)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                {title && <h3 style={{ fontSize: '20px', fontWeight: 600, color: '#2E3440' }}>{title}</h3>}
                <span style={{ padding: '4px 12px', background: 'linear-gradient(135deg, #5E81AC, #81A1C1)', color: '#fff', borderRadius: '20px', fontSize: '13px', fontWeight: 500 }}>放大视图</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', position: 'relative' }}>
                <div className="relative group">
                  <button disabled={isExporting} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px', fontSize: '14px', fontWeight: 500, border: 'none', cursor: isExporting ? 'not-allowed' : 'pointer', transition: 'all 0.3s', background: isExporting ? '#D8DEE9' : 'linear-gradient(135deg, #A3BE8C, #8FBCBB)', color: '#fff' }} title="下载图表">
                ...
                <button onClick={() => { setIsModalOpen(false); setIsExporting(false); setExportStatus(''); }} style={{ padding: '10px', borderRadius: '12px', border: 'none', cursor: 'pointer', transition: 'all 0.3s', backgroundColor: '#E5E9F0' }} title="关闭"><X style={{ width: '20px', height: '20px', color: '#4C566A' }} /></button>
              </div>
            </div>
            <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '24px' }}>
                {CHART_TYPES.map(({ type, label, icon }) => (
                  <button key={type} onClick={() => setChartType(type)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px', borderRadius: '12px', fontSize: '14px', fontWeight: 500, border: 'none', cursor: 'pointer', transition: 'all 0.3s', background: chartType === type ? 'linear-gradient(135deg, #5E81AC, #81A1C1)' : '#E5E9F0', color: chartType === type ? '#fff' : '#4C566A' }}>
                    {icon}<span>{label}</span>
                  </button>
                ))}
              </div>
              ...
              {chartType !== 'pie' && <ChartLegend dataKeys={dataKeys} hiddenSeries={hiddenSeries} getSeriesConfig={getSeriesConfig} toggleSeries={toggleSeries} />}
              <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px solid #D8DEE9' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '8px', fontSize: '14px', color: '#4C566A' }}>
